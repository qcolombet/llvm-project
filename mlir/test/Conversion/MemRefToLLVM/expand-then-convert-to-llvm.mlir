// RUN: mlir-opt -simplify-extract-strided-metadata -convert-memref-to-llvm -lower-affine -convert-arith-to-llvm -cse %s -split-input-file | FileCheck %s
...
// The final code is still not perfect, becase we have
...
-> memref<?x?xf32, strided<[?, ?], offset: ?>> {
  // CHECK-DAG: %[[BASE:.*]] = builtin.unrealized_conversion_cast %[[MEM]]
  // CHECK-DAG: %[[BASEREF:.*]] = llvm.extractvalue %[[BASE]][0]
  // CHECK-DAG: %[[MEMREF:.*]] = builtin.unrealized_conversion_cast %[[BASEREF]]
...
func.func @subview_non_zero_addrspace(%0 : memref<64x4xf32, strided<[4, 1], offset: 0>, 3>, %arg0 : index, %arg1 : index, %arg2 : index) -> memref<?x?xf32, strided<[?, ?], offset: ?>, 3> {
  ...
  // CHECK: %[[BASE:.*]] = llvm.extractvalue %[[MEMREF]][0] : !llvm.struct<(ptr<f32, 3>, ptr<f32, 3>, i64)>
  // CHECK: %[[BASE_ALIGNED:.*]] = llvm.extractvalue %[[MEMREF]][1] : !llvm.struct<(ptr<f32, 3>, ptr<f32, 3>, i64)>
...
func.func @subview_const_stride_and_offset(%0 : memref<64x4xf32, strided<[4, 1], offset: 0>>) -> memref<62x3xf32, strided<[4, 1], offset: 8>> {
  ...
  // CHECK: %[[BASE:.*]] = builtin.unrealized_conversion_cast %[[MEM]]
  // CHECK: %[[BASEREF:.*]] = llvm.extractvalue %[[BASE]][0]
  // CHECK: %[[MEMREF:.*]] = builtin.unrealized_conversion_cast %[[BASEREF]]
...
func.func @subview_leading_operands_dynamic(%0 : memref<5x?xf32>) -> memref<3x?xf32, strided<[?, 1], offset: ?>> {
  // CHECK: %[[BASE:.*]] = builtin.unrealized_conversion_cast %[[MEM]]
  // CHECK: %[[SIZE1:.*]] = llvm.extractvalue %[[BASE]][3, 1] : !llvm.struct<(ptr<f32>, ptr<f32>, i64, array<2 x i64>, array<2 x i64>)>
  // CHECK: %[[BASEREF:.*]] = llvm.extractvalue %[[BASE]][0]
  // CHECK: %[[MEMREF:.*]] = builtin.unrealized_conversion_cast %[[BASEREF]]
  // Extract strides
  // CHECK: %[[STRIDE0:.*]] = llvm.extractvalue %[[BASE]][4, 0] : !llvm.struct<(ptr<f32>, ptr<f32>, i64, array<2 x i64>, array<2 x i64>)>
